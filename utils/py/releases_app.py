    ...
    try:
        ...
        def mutate(conn):
            ...
            for block in blocks:
                ...
                # hasn't shipped a version yet (RELEASES.md's own header: "Versions are RECORDED,
                # never RESERVED... Release: reads TBD until a release's last issue closes"). The
                # upstream schema already supports "no version yet" as SQL NULL (releases.version is
                # nullable and UNIQUE(repo_id, version) permits any number of NULLs), it just never
                # maps the literal string "TBD" to it. Map it here so multiple unshipped releases can
                # coexist, without weakening uniqueness for any real version string.
                if version == "TBD":
                    version = None
                gid = new_gid("rel-")
