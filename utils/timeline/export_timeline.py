...
def release_columns(cx, repo_url, roadmap_idx, today):
    ...
    # AEGIS-SLEUTH DEVIATION from upstream export_timeline.py (not present in XYZ-forge): the
    # secondary sort key is r[1] (version), which is SQL NULL / Python None for any unversioned
    # release. Upstream's own ledger apparently never had two+ releases tied on the primary key
    # (same shipped/cut-vs-open bucket + same date) while also mixing a None version in with a
    # real version string, so `str < NoneType` never fired there. This repo has multiple
    # unversioned open releases with no target_date (all fall into the same (1, "9999-12-31")
    # bucket), which crashes the sort. `r[1] or ""` makes an unversioned release sort before any
    # real version string, which is an arbitrary but stable tiebreak — no functional meaning is
    # implied by it.
    rows.sort(
        key=lambda r: (
    ...
    for (rid, version, codename, status, target, shipped, descr, exit_c, milestone, mar_id,
        ...
        columns.append(
            {
                # AEGIS-SLEUTH DEVIATION: same None-version gap as the sort key above — `version`
                # is None for any unversioned release (this repo has several). Falls back to
                # codename, then the release's own internal DB row id, so every column still gets
                # a distinct, stable `id` instead of crashing on `None.replace(...)`.
                "id": "c-" + (version or codename or ("r%d" % rid)).replace(".", "-"),
...
def _ver(v):
    # AEGIS-SLEUTH DEVIATION: broadened to also catch AttributeError. This repo has many
    # unversioned (None) releases, which `.split(".")` raises AttributeError on rather than
    # ValueError — a case upstream's own ledger apparently never hits at this call site. Treating
    # "not a version at all" the same as "malformed version string" matches this function's
    # existing intent (gracefully return None, don't crash the caller).
    try:
