...
| `data/static/ai/command-normalization.json` | Preprocessing rewrites applied *before* route dispatch: model-name aliases (`ModelAliases`) and `DirectCommandPatterns`. **`ModelAliases` is GENERATED** (GH-173): `scripts/sync-model-catalog.js` writes it from the vendored HiQS-Labs/Model-catalog copy (`model-catalog.json` + `model-catalog.pin.json` beside it); never hand-edit it — add the row upstream, then `npm run sync:model-catalog -- --tag vX.Y.Z`; `npm run validate:model-catalog` and `tests/model-catalog-sync.test.js` turn red on a hand edit. Every other key is hand-maintained. `DirectCommandPatterns` match command-syntax variations (e.g. `model switch:default='X'`, `show channel model` with spaces) and canonicalize them into the hyphenated form the router expects — they are not NL discovery and do not help users find commands they don't know about. Add here when a typed command has multiple valid syntaxes; add to `command-catalog.json` RegexAliases when adding a new NL phrasing for a known command. |
