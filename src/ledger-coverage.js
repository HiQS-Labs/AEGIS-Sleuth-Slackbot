...
function BuildLedgerCoverage(ArgOptions) {
  ...
  async function WriteMarkerAsync(ArgWorkspace, ArgMarker) {
    MarkerCache.set(ArgWorkspace, ArgMarker);
    try {
      await fs.mkdir(RootDir, { recursive: true });
      await WriteFileDurableAsync(CoverageFilePath(RootDir, ArgWorkspace), JSON.stringify(ArgMarker, null, 2), { Logger });
    } catch(error) {
      // Never throw at the call site — this sits on the ledger path, which must not break a
      // reminder transition. The in-memory cache still reflects the gap for this process, and the
      // gate stays closed; the risk is only that a restart forgets it, which is why the failure is
