...
- **Never edit a vendored XYZ / PDDA / PRS file in place, and never commit a copy of one into this repo.** `.xyz/` comes from XYZ-forge and `utils/pdda/` from PDDA; both are replaced wholesale on the next sync, so a local edit is a fork that rots silently. Behavior that must differ here goes **upstream as a setting**: file an issue on the tool's repo, land the setting, revendor, then configure it from this side. See §15 for what this cost us the one time it was ignored.
...
- **Editing a vendored XYZ / PDDA / PRS file in place, or committing a copy of one into this repo.** Those tools are vendored from their own upstreams (`.xyz/` from XYZ-forge, `utils/pdda/` from PDDA); a local edit is a fork, and a fork silently rots. If behavior must differ here, it goes **upstream as a setting** — file an issue on the tool's repo, land the setting there, revendor, and configure it from this side. Never patch the vendored copy and never keep a tracked duplicate "so we can edit it". Evidence: `utils/py/releases_app.py` was forked for two lines (`Release: TBD` -> SQL NULL), then drifted ~1700 lines behind upstream, lost four `roadmap` verbs the rest of the repo needed, and caused an already-fixed upstream bug to be re-derived and re-fixed here as GH-183/#184 — an issue, a PR and a review cycle spent on nothing. Removed in GH-187; the behavior is now `unshipped_version_tokens` in the ledger's own settings (XYZ-forge GH-525).
