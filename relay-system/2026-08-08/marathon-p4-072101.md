...
Last recorded reason (<repo-root>/phases/ledger-p3-entity-linking--p4/ESCALATION.md): `containment-violation (off-lane edit reverted by a turn-taker)`. Read it before re-guessing.
...
3. Use this exact tick binary (run it from any directory): <repo-root>/.xyz/bin/tick
   - <repo-root>/.xyz/bin/tick claim MARATHON-P4-TURN-2 --agent codex --paths "phases/ledger-p3-entity-linking--p4/RELAY.md,scripts/entity-linking-diagnostics.js,tests/entity-linking-diagnostics.test.js"
   - <repo-root>/.xyz/bin/tick ping MARATHON-P4-TURN-2 --agent codex
   - <repo-root>/.xyz/bin/tick release MARATHON-P4-TURN-2 --agent codex --to agy
...
2. If changes needed: add `**Verdict:** Changes requested` then: <repo-root>/.xyz/bin/tick release MARATHON-P4-TURN-2 --agent agy --to codex
3. If satisfied: add `**Verdict:** Approved`, set `STATUS: Approved`, then: <repo-root>/.xyz/bin/tick done MARATHON-P4-TURN-2 --agent agy
