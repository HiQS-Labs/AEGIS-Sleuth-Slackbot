...
| R7 | Corpus through all four `Simulate*` hooks: never reject, never a handler-error log entry, `setImmediate` drain so a fire-and-forget rejection is attributed to the test (**R7 as amended by the owner on 2026-09-05**) | block 3 | the three-part invariant in `ExpectHandledCleanly`; the drain is the mechanism and jest-circus is the enforcement. See "Fire-and-forget rejections under jest" for the four probes behind the amendment |
