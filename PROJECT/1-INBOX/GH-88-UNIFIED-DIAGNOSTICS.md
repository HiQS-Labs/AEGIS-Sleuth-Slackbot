...
- **Knobs:** `ModelAliases` (`data/static/ai/command-normalization.json` — since GH-173 a GENERATED section synced from HiQS-Labs/Model-catalog, turned upstream rather than edited here), provider prefix patterns, default-provider fallback, `COMMAND_NEAR_MISS_LLM`, router mode.
